...
exports.getOrdersForUser = async (req, res, next) => {
  try {
    ...
    const orders = await Order.find({ user: userId }).populate('address').exec();
...
exports.getFavoritesByUserId = async (req, res, next) => {
  try {
    ...
    const favorites = await Favorites.findOne({ user: userId }).populate('products').exec();

    if (!favorites) {
      return res.status(404).json({ message: 'Favorites not found for the user' });
    }
...
exports.addToFavorites = async (req, res, next) => {
  try {
    ...
    let favorites = await Favorites.findOne({ user: userId }).exec();
    if (!favorites) {
...
exports.removeFromFavorites = async (req, res, next) => {
  try {
    ...
    favorites.products = favorites.products.filter((p) => p.toString() !== productId);
    await favorites.save();
